...
def print_install_guide(goodie_name: str):
    latest_commit_p = subprocess.run(["git", "rev-parse", "HEAD"], text=True, capture_output=True, check=True, cwd=REPO_ROOT)
...
def regenerate_readme():
    with (REPO_ROOT / 'README.md').open('r') as readme:
        ...
        if not found_goodies_section:
            print("<!-- START GOODIES SECTION --> missing from README.md", file=sys.stderr)
